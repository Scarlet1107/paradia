...
type Database = SupabaseClientType extends SupabaseClient<infer U> ? U : any;
...
type SupabaseTableName = Extract<keyof DatabaseSchema["Tables"], string>;
  ...
  const fetchPage = async (skip: number) => {
    ...
    if (error) {
      ...
      const deduplicatedData = (
        (newData || []) as (TData & { id: string | number })[]
      ).filter(
        (item) => !state.data.find((old) => (old as any).id === item.id),
      );
      setState({
